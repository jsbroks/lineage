...
import { determineLotTypeCode } from "~/lib/lot-type-code";
...
type Props = {
  initialData?: LotTypeFormData;
  onSubmit: (data: LotTypeFormData) => Promise<void>;
  isSubmitting: boolean;
  submitLabel: string;
};

export function LotTypeForm({
  initialData,
  onSubmit,
  isSubmitting,
  submitLabel,
}: Props) {
  const [base, setBase] = useState<LotTypeFormValues>(
  ...
  const handleNameChange = (value: string) => {
    setBase((prev) => {
      ...
      if (!codePrefixTouched.current) {
        next.codePrefix = determineLotTypeCode({
          name: value,
          slug: slugify(value),
        });
      }
