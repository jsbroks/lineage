  ...
  return (
    <div className="container mx-auto max-w-6xl px-6 py-8">
      <div className="mb-6 flex items-center justify-between gap-2">
        <div>
          <Badge variant="outline">{data.lotType?.name}</Badge>
          <h1 className="text-2xl font-semibold tracking-tight">
            {data.lot.code}
          </h1>
          <p className="text-muted-foreground mt-1 text-sm">
            Status: {data.lot.statusId}
          </p>
        </div>
        <Link
          href={`/${org}/lots/new`}
        ...
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            <p>
              ...
              {data.lotType?.name ?? data.lot.lotTypeId}
            </p>
            <p>
        ...
        <Card>
          ...
          <CardContent>
            {data.parentLineage.length === 0 ? (
              <p className="text-muted-foreground text-sm">
                No source lots recorded.
              </p>
            ) : (
              <div className="space-y-2">
                {data.parentLineage.map(({ link, lot: parentLot }) => (
                  <div
                    key={link.id}
                    className="border-border rounded-md border p-2 text-sm"
                  >
                    <p className="font-medium">
                      {parentLot?.code ?? link.parentLotId}
                    </p>
                    <p className="text-muted-foreground text-xs">
        ...
        <Card>
          ...
          <CardContent>
            {data.childLineage.length === 0 ? (
              ...
              <div className="space-y-2">
                {data.childLineage.map(({ link, lot: childLot }) => (
                  <div
                    key={link.id}
                    className="border-border rounded-md border p-2 text-sm"
                  >
                    <p className="font-medium">
                      {childLot?.code ?? link.childLotId}
                    </p>
                    <p className="text-muted-foreground text-xs">
                      Relation: {link.relationship}
                    </p>
