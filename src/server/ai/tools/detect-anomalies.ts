...
export function createDetectAnomaliesTool(_ctx: SchemaContext) {
  return tool({
    description:
      "Detect anomalies in the inventory: stuck items (in a status much longer than peers), throughput drops (fewer status transitions this week vs last week), and yield outliers (unusual quantities by type/variant). Call this when the user asks about problems, issues, anomalies, stuck items, things that need attention, or anything unusual.",
    inputSchema: z.object({
      scope: z
        .enum(["all", "stuckItems", "throughputChanges", "yieldOutliers"])
        .default("all")
        .describe(
