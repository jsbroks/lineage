...
export function createAggregateItemsTool(ctx: SchemaContext) {
  return tool({
    description:
    ...
    execute: async ({ itemTypeName, groupBy, metrics, filters }) => {
      ...
      for (const field of groupBy) {
        ...
        if (resolved.kind === "builtin") {
          switch (resolved.key) {
            case "status":
              groupByExprs.push({ key: "status", expr: sql`${item.statusId}` });
              break;
            case "variant":
          ...
          groupByExprs.push({
            key: field,
            expr: sql`${item.attributes}->>${sql.raw(`'${escapedKey}'`)}`,
          });
        }
      }
